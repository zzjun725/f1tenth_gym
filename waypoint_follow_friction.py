...
def main():
    ...
    main entry point
    """
    # vels_ = [13, 14.7, 14.9, 15.1, 15.3, 16]
    vels_ = [7.7, 7.9, 8.1, 8.3]
    frictions_ = [0.6, 0.8, 1.0, 1.2, 1.4]

    # for i_vel in np.arange(5., 9.2, 0.2):
    #     vel_ = round(i_vel, 1)
    for vel_ in vels_:
        for friction_ in frictions_:
            ...
            obs, step_reward, done, info = env.reset(
                np.array([[0., 0., 3.4, 0.0, 0.0, 0.0, 0.0]]))

            # # NOTE: initialized states for reverse
            # obs, step_reward, done, info = env.reset(
            #     np.array([[0., 0., 3.4+3.14, 0.0, 0.0, 0.0, 0.0]]))
            
            # env.render() # NOTE: render
            ...
            #                         work['vgain']])
            state = np.array([obs['x4'][0], obs['x6'][0], obs['x11'][0], 
                            obs['x16'][0], obs['x21'][0]])
            ...
            file_name_ = 'data/Spielberg_raceline/' # NOTE
            np.save(file_name_+'states_mb_fric_{}_vel_{}.npy'.format(int(friction_*10), int(vel_*10)), states)
            np.save(file_name_+'controls_mb_fric_{}_vel_{}.npy'.format(int(friction_*10), int(vel_*10)), controls)
